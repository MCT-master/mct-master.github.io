...
For this scene an ambisonic recording from the train station Lilleby was used and stereo/mono placed recordings around it. The footsteps for example or the breathing (dog breathing turned out to be the most realistic for the running sequence).  

### Forest
The person runs into the forest and goes for a walk. There it escapes from a storm in a Cave.  
...
### Shore Scene
Being transported from the cave through the portal, the spinning, filtered sounds imply the dizziness and disorientation. Then the full range hearing comes back and the ambient sounds can be heard (Sea and Sea Guls). The intention was to use an IR reverb, but regarding the unpleasant results from the beach impulse response, and the sounds being reverberant enough, it was not used.  
...
We realized that it is often better to recreate illusions like a forest for example through samples instead of going out for field recordings and being limited by the large acoustic arena of transportation and vehicle sounds, that are present in every corner of the world.  
